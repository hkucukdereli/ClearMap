...
removeBackgroundParameter = {
    "size"    : (10,10),  # size in pixels (x,y) for the structure element of the morphological opening
...
filterDoGParameter = {
    "size"    : (5,5),        # (tuple or None)      size for the DoG filter in pixels (x,y,z) if None, do not correct for any background
...
VoxelizationFile = os.path.join(BaseDirectory, 'AER_points_voxelized.tif');
...
CorrectionResamplingParameterCfos["sink"]   = os.path.join(BaseDirectory, 'AER_cfos_resampled.tif');
...
CorrectionResamplingParameterAutoFluo["sink"]   = os.path.join(BaseDirectory, 'AER_autofluo_for_cfos_resampled.tif');
   
#Files for Auto-fluorescence (Atlas Registration)
RegistrationResamplingParameter = CorrectionResamplingParameterAutoFluo.copy();
RegistrationResamplingParameter["sink"]            =  os.path.join(BaseDirectory, 'AER_autofluo_resampled.tif');
...
CorrectionAlignmentParameter = {            
    ...
    "movingImage" : os.path.join(BaseDirectory, 'AER_autofluo_for_cfos_resampled.tif'),
    "fixedImage"  : os.path.join(BaseDirectory, 'AER_cfos_resampled.tif'),
    
    #elastix parameter files for alignment
    "affineParameterFile"  : os.path.join(PathReg, 'Parameter_files/Par0000affine_acquisition.txt'),
    "bSplineParameterFile" : None,
    
    #directory of the alignment result
    "resultDirectory" :  os.path.join(BaseDirectory, 'AER_elastix_cfos_to_auto')
    }; 
...
RegistrationAlignmentParameter["resultDirectory"] = os.path.join(BaseDirectory, 'AER_elastix_auto_to_atlas');
    
#moving and reference images
RegistrationAlignmentParameter["movingImage"]  = AtlasFile;
RegistrationAlignmentParameter["fixedImage"]   = os.path.join(BaseDirectory, 'AER_autofluo_resampled.tif');

#elastix parameter files for alignment
RegistrationAlignmentParameter["affineParameterFile"]  = os.path.join(PathReg, 'Parameter_files/Par0000affine.txt');
RegistrationAlignmentParameter["bSplineParameterFile"] = os.path.join(PathReg, 'Parameter_files/Par0000bspline.txt');
...
SpotDetectionParameter = {
    "source" : cFosFile,
    "sink"   : (os.path.join(BaseDirectory, 'AER_voxels-allpoints.npy'),  os.path.join(BaseDirectory,  'AER_intensities-allpoints.npy')),
...
FilteredCellsFile = (os.path.join(BaseDirectory, 'AER_voxels.npy'), os.path.join(BaseDirectory,  'AER_intensities.npy'));

TransformedCellsFile = os.path.join(BaseDirectory, 'AER_voxels_transformed_to_Atlas.npy')
...
CorrectionResamplingPointsParameter["pointSource"] = os.path.join(BaseDirectory, 'AER_voxels.npy');
